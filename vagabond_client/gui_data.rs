...
use ggez::graphics::{Color, DrawParam, Text, TextFragment, Font};
...
impl Clock {
    ...
    pub fn draw(&self, ctx: &mut Context, font: &Font) -> GameResult {
        ...
        let text_fragment = TextFragment::new(self.current.to_string()).scale(scale).font(font.clone());
        ...
        let mut mesh_builder = graphics::MeshBuilder::new();
        let text_background_rect = graphics::Rect::new(location.x, location.y, timer_text_width, timer_text_height);
        let rect_mesh = mesh_builder.rectangle(graphics::DrawMode::Fill(graphics::FillOptions::default()), text_background_rect, Color::new(0.25, 0.25, 0.25, 0.75)).build(ctx).unwrap();


        graphics::draw(ctx, &rect_mesh, DrawParam::new()).unwrap();

        graphics::draw(ctx, timer_text.set_font(font.clone(), scale), DrawParam::new().dest(location.as_mint_point())).unwrap();
